...
public class EmotionPieDataProcessor extends DataProcessor {
	
	ArrayList<Data> pie_data;
	
	public EmotionPieDataProcessor(){
		super();
		this.data_type_raw = JOURNAL;
		this.pie = new Pie_Arc(0, 0, 0);
	...
	public void loadPie(Date date_range_start, Time time_range_start, Date date_range_end, Time time_range_end) {
		pie = new Pie_Arc(processing_app.width/2-100, processing_app.height/2, (float) ((processing_app.height/2)*0.75));
		pie_data = load_date_and_time_range(EMOTION, date_range_start, time_range_start, date_range_end, time_range_end);
		pie.setName("Emotions");
		for (int i = 0; i < pie_data.size(); i++) {
			DataPieEmotion pie_record = (DataPieEmotion) pie_data.get(i);
			PApplet.println("LOAD PIE " + i + " info " + pie_record.getString());

			int h = 0;
			if (pie_record.emotion_type > 0 ) h = 220;
			int s = (int) ( ( (pie_record.emotion_intensity/4.0) + 0.6)* 255);
			int b = 255;
			int cur_color = ShapeColor.colorHSB(h, s, b);
			cur_color = ShapeColor.colorRGB(cur_color);

			pie.addSlice(pie_record.name, pie_record.description, 0, cur_color);
			pie.setSliceValue(i, Time.calculate_time_dif_seconds(pie_record.time_stamp, pie_record.time_end));
			pie.setColorSlice(i, cur_color);
		}
		pie.setShiftMouseOverAll((float) 0.15, (float) -0.15, (float) 0.15, (float) 0.1, true);
		pie.textLocationNameDescription((float)(processing_app.width*0.85)-100, (float)(processing_app.height*0.5), 0, 1200);
		pie.textLocationNameDescriptionSlices((float)(processing_app.width*0.85)-100, (float)(processing_app.height*0.5+30), 0, 20);
		pie.loadFontAll(1, 20);
		...
		for(int i = 0; i < this.data_list_processed.size(); i++) {
			...
			if (current_journal_entries.size() >  0) { 
				...
				for(int k = current_journal_entries.size() - 1; k >= 0 ; k--) {
					...
					emotion_text = joinStringsWithLists(emotion_text, (current_journal_entry.emotion_L1 + ", " + current_journal_entry.emotion_L2 + ", " + current_journal_entry.emotion_L3));
				...
				if (last_journal_entry_age < DataVizElement.EmotionAgeThreshold) {
					...
					emotion_text = joinStringsWithLists(emotion_text, (last_journal_entry.emotion_L1 + ", " + last_journal_entry.emotion_L2 + ", " + last_journal_entry.emotion_L3));
				...
				current_emotion_reading.setName("feeling " + convertEmotionToString(current_emotion_reading.emotion_type) + ", " + convertEmotionIntensityToString(current_emotion_reading.emotion_intensity) + " intensity");
				...
				current_emotion_reading.setDescription("feeling " + current_emotion_reading.emotion.toLowerCase() + "\n" +
				...
				if (last_journal_entry_age < EmotionAgeThreshold) {
					...
					current_emotion_reading.setName(convertEmotionToString(current_emotion_reading.emotion_type) + " mood, " + convertEmotionIntensityToString(current_emotion_reading.emotion_intensity) + " intensity");
					current_emotion_reading.setEmotion(emotion_text);
					current_emotion_reading.setActivity(activity_text);
					current_emotion_reading.setPeople(people_text);
					current_emotion_reading.setPlace(place_text);
