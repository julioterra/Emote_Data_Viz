...
public class PieArcSlice extends PieSlice{
		  
	  /*********************************************************
	   ** CONSTRUCTOR METHODS 
	   **/
	
	  public PieArcSlice(int x_loc, int y_loc, float radius, float size_in_percent, float angle_start, int color) {
		  super(x_loc, y_loc, radius, size_in_percent, angle_start, color);
		  this.pie_shape_type = PIE_ARC_SLICE;
		  this.setRadiusShift((float) 0.1);
		  this.setColorShift((float) 0.15, (float) -0.15, (float) 0.15);
	  }
